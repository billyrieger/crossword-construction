...
/// An instance in time during the search procedure.
#[derive(Clone, Copy, Debug, Default, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct Timestamp(pub usize);

fn save_trail<T>(now: Timestamp, value: &T, initial: &T, history: &mut Vec<(Timestamp, T)>)
where
    T: Clone + PartialEq,
{
    if value != history.last().map(|(_, t)| t).unwrap_or(initial) {
        history.push((now, value.clone()));
    }
}

fn backtrack_trail<T>(
    then: Timestamp,
    value: &mut T,
    initial: &T,
    history: &mut Vec<(Timestamp, T)>,
) where
    T: Clone + PartialEq,
{
    while let Some((saved_at, prev_value)) = history.pop() {
        if saved_at <= then {
            *value = prev_value.clone();
            history.push((saved_at, prev_value));
            return;
        }
    }
    *value = initial.clone();
...
{
    fn save(&mut self, now: Timestamp) {
        save_trail(now, &self.value, &self.initial, &mut self.history);
    }

    fn backtrack(&mut self, then: Timestamp) {
        backtrack_trail(then, &mut self.value, &self.initial, &mut self.history);
    }
}
...
impl State for SparseBitSet {
    ...
    fn backtrack(&mut self, then: Timestamp) {
        self.indices.backtrack(then);
        for &index in self.indices.values() {
            self.words[index].backtrack(then);
        }
    }
}
