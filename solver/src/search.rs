...
impl State for SearchState {
    ...
    fn backtrack(&mut self, then: Timestamp) {
        self.cells.backtrack(then);
        self.entries.backtrack(then);
        self.tables.iter_mut().for_each(|c| c.backtrack(then));
        self.alldiffs.iter_mut().for_each(|c| c.backtrack(then));
    }
}
